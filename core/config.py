...
def load_config():
    global XP_PER_RUN_DEFAULT, TARGET_LEVEL, DEBUG_MODE, PROFILE_CACHE_TTL, PRICES_CACHE_TTL, OWNER_IDS, CONGRATS_GIFS
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, 'r') as f:
                ...
                PROFILE_CACHE_TTL = data.get("profile_cache_ttl", PROFILE_CACHE_TTL)
                PRICES_CACHE_TTL = data.get("prices_cache_ttl", PRICES_CACHE_TTL)
                
                if "owner_ids" in data:
                    OWNER_IDS = data["owner_ids"]
                if "congrats_gifs" in data:
                    CONGRATS_GIFS = data["congrats_gifs"]
...
def save_config():
    data = {
        ...
        "prices_cache_ttl": PRICES_CACHE_TTL,
        "owner_ids": OWNER_IDS,
        "congrats_gifs": CONGRATS_GIFS
    }
    try:
